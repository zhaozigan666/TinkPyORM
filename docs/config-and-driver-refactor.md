...
> **v0.3.1 修订**：本文的"配置中心化"部分（Phase 1：Config 值对象 + DatabaseManager
> 注册表 + DSN/env/file 来源）已按使用反馈**撤销**——对单库 SQLite 场景过于复杂，
> 配置入口收敛回 `Db.set_config`（见提交 v0.3.1）。**驱动抽象部分（Phase 2）已落地并保留**
> （`tinkpyorm/drivers/`）。后续新增数据库只需实现驱动并注册，无需配置层参与。
